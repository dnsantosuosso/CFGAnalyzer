...
public class Main {
	...
	private boolean isPathExist() {
		 ...
		 */
		return Files.exists(Paths.get(this.getPath()));
	}
	...
	private void printHelp(Options options) {
		...
		String footer ="";
		formatter.printHelp("execute", header, options, footer, true);
	}	
	
	public static void main (String [] args) throws Exception {
		Main scan = new Main();
		CFGAnalyzer execute = new CFGAnalyzer();
...
//		System.out.print(scan.getPath());
		...
		// 2. Option 'n'
		if(scan.nBranch == true) {
			ArrayList<ArrayList<Integer>> branches = execute.cfg.branches(1);
			System.out.println();
			System.out.println("nBranch: " + branches.size());
		}
		
		// 3. Option 'd'
		if(scan.showBranch == true) {
			System.out.println();
			System.out.print("Branches:");
			
			
			ArrayList<ArrayList<Integer>> branch = execute.cfg.branches(1);
			for(ArrayList<Integer> b : branch) {
				System.out.print("\n" + b.get(0));
				for(int i = 1; i < b.size(); i++) {
					String to_print = "";
					to_print= to_print + " -> " + b.get(i);
